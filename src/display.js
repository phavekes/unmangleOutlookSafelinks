...
function removeSenderIdentificationTable() {
  // 1. Find the specific link inside that annoying table
  const links = document.querySelectorAll('a[href*="aka.ms/LearnAboutSenderIdentification"]');
  
  links.forEach(link => {
    // 2. Look up the DOM tree for the nearest <table> element
    const table = link.closest('table');
    
    // 3. If we found a table containing this link, delete the whole thing
    if (table) {
